...
const paramsSchema = z.object({
    ...
    files: z.array(
        z.object({
            name: z.string().min(1, "File name connot be empty"),
            content: z.string()
        })
    )
...
export const createCreateFilesTool = ({ internalKey, projectId }: CreateFilesToolOptions) => {
    ...
    return createTool({
        ...
        parameters: z.object({
            parentId: z.string().describe("The ID of of the parent folder. Use empty string for root level. Must be a valid folder ID from listFiles."),
            files: z.array(
                z.object({
