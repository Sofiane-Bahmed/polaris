...
export const useWebContainer = ({
    ...
    //fetch files from convex (auto-updated on changes)
    const files = useFiles(projectId);

    //Initial boot and mount
    useEffect(() => {

        if (!enabled || !files || files.length === 0 || hasStartedRef.current) return;
    ...
    //Resatrt the entire webConatiner process
