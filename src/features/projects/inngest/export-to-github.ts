...
export const exportToGithub = inngest.createFunction(
    ...
    async ({ event, step }) => {
        const {
            ...
            description,
            githubToken,
        } = await event.data as ExportToGithubEvent;
        ...
        // Get the intial commit SHA (we need this for our commit)
        ...
        if (treeItems.length === 0) {
            throw new NonRetriableError("Failed to create any file blob");
        }
